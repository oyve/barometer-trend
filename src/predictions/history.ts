 ...
 */
export function getHistoricPressures(limit: number = 48): HistoricPressure[] {
	const historicPressures: HistoricPressure[] = [];

	for (let hour = 1; hour <= limit; hour++) {
		const threshold = utils.minutesFromNow(-hour * 60);

		const pressure = readingStore.getPressureClosestTo(threshold);
